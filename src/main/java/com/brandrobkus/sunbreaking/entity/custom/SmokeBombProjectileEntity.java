...
public class SmokeBombProjectileEntity extends ThrownItemEntity {
    ...
    private static final TrackedData<ItemStack> SMOKE_STACK =
            DataTracker.registerData(SmokeBombProjectileEntity.class, TrackedDataHandlerRegistry.ITEM_STACK);
    ...
    public SmokeBombProjectileEntity(World world, LivingEntity owner) {
        ...
        this.dataTracker.set(SMOKE_STACK, owner.getMainHandStack().copy());
    }

    public SmokeBombProjectileEntity(World world, double x, double y, double z) {
        super(ModEntities.SMOKE_BOMB, x, y, z, world);
    }

    public ItemStack getBaseSmokeStack() {
        return this.dataTracker.get(SMOKE_STACK);
    }

    @Override
    protected void initDataTracker() {
        super.initDataTracker();
        this.dataTracker.startTracking(SMOKE_STACK, ItemStack.EMPTY);
    }
    ...
    private boolean entityHasObscuring(LivingEntity entity) {
        if (!(entity instanceof PlayerEntity player)) return false;

        ItemStack chest = player.getInventory().getArmorStack(2);
        if (!(chest.getItem() instanceof ModVoidArmorItem voidItem)) {
            return false;
        }

        return ModVoidArmorItem.hasItemInBundle(chest, ModItems.ASPECT_OF_OBSCURITY);
    }

    private boolean areFireteamMembers(LivingEntity a, LivingEntity b) {
        if (!(a instanceof ServerPlayerEntity spA)) return false;
        if (!(b instanceof ServerPlayerEntity spB)) return false;

        return FireteamManager.areTeammates(spA, spB);
    }
    ...
    private boolean applyInvisibilityAoE() {
        ...
        boolean ownerHasObscuring = entityHasObscuring(owner);
